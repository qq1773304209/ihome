...
class User(BaseModel, db.Model):
    ...
    def to_dict(self):
        user_dict = {
            "user_id": self.id,
            "name": self.name,
            "avatar_url": current_app.config.get("QINIU_URL_DOMAIN") + self.avatar_url if self.avatar_url else "",
            "mobile1": self.mobile,
...
class House(BaseModel, db.Model):
    ...
    def to_basic_dict(self):
        ...
        house_dict = {
            ...
            "img_url": current_app.config.get("QINIU_URL_DOMAIN") + self.index_image_url if self.index_image_url else "",
            "room_count": self.room_count,
            "order_count": self.order_count,
            "address": self.address,
            "user_avatar": current_app.config.get("QINIU_URL_DOMAIN") + self.user.avatar_url if self.user.avatar_url else "",
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_urls.append(current_app.config.get("QINIU_URL_DOMAIN") + image.url)
        ...
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None) \
            .order_by(Order.update_time.desc()).limit(current_app.config.get("HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS"))
        for order in orders:
            comment = {
