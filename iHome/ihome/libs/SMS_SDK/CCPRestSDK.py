...
class REST:
    ...
    def sendTemplateSMS(self, to,datas,tempId):
        ...
        #生成auth
        src = self.AccountSid + ":" + self.Batch;
        auth = base64.b64encode(bytes(src, 'utf-8')).strip() # base64.b64encode(bytes(temp, 'utf-8'))
        req = urllib.request.Request(url)
        self.setHttpHeader(req)
        req.add_header("Authorization", auth)
        #创建包体
        ...
        # body = '<?xml version="1.0" encoding="utf-8"?><TemplateSMS>' \
        #        '<to>%s</to>' \
        #        '<appId>%s</appId> ' \
        ...
        print(body)
        dic = xmltodict.parse(body)
        print(dic)
        d = bytes(urllib.parse.urlencode(dic), encoding='utf8')
        data=''
        try:
            res = urllib.request.urlopen(req, data=d)
            data = res.read()
            res.close()
