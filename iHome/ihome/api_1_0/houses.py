...
def get_area_info():
    ...
    try:
        ...
        if resp_json is not None:
            # 有缓存
            current_app.logger.info("hit readis area_info")
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(error=RET.DBERR, errmsg="查询数据库错误")
    ...
    # 数据存入redis
    try:
        redis_store.setex("area_info", current_app.config.get("AREA_INFO_REDIS_CACHE_EXPIRES"), resp_json)
    except Exception as e:
        current_app.logger.error(e)
    # 返回
    return resp_json, 200, {"Content-Type": "application/json"}
...
def save_house_image():
    ...
    # 保存图片到七牛中
    try:
        file_name = storage_file(image_data)
    except Exception as e:
        current_app.logger.error(e)
    ...
    image_url = current_app.config.get("QINIU_URL_DOMAIN") + file_name

    return jsonify(errno=RET.OK, errmsg="OK", data={"image_url": image_url})

@api.route("user/houses", methods=["POST"])
@login_required
def get_user_houses():
    '''获取房源信息'''
    ...
    # 查看数据库
    try:
        houses = House.query.filter_by(id=user_id).all()
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(error=RET.DBERR, errmsg="数据库错误")

    if houses is None:
        return jsonify(error=RET.NODATA, errmsg="无房源")
    ...
    return jsonify(error=RET.OK, errmsg="OK", data={"houses_li": houses_li})
...
def get_house_index():
    ...
    if ret:
        ...
        try:
            ...
            houses = House.query.order_by(House.order_count.desc()).limit(current_app.config.get("HOME_PAGE_MAX_HOUSES"))
        except Exception as e:
            current_app.logger.error(e)
        ...
        try:
            redis_store.setex("home_page_data", current_app.config.get("HOME_PAGE_DATA_REDIS_EXPIRES"), json_houses)
        except Exception as e:
            current_app.logger.error(e)
...
def get_house_detail(house_id):
    ...
    try:
        redis_store.setex("house_info_%s" % house_id, current_app.config.get("HOUSE_DETAIL_REDIS_EXPIRE_SECOND"), json_house)
    except Exception as e:
        current_app.logger.error(e)
...
def get_house_list():
    ...
    try:
        #                               当前页数          每页数据量                                             自动的错误输出
        page_obj = house_query.paginate(page=page, per_page=current_app.config.get("HOUSE_LIST_PAGE_CAPACITY"), error_out=False)
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="数据库异常")
    ...
    if page <= total_page:
        ...
        try:
            ...
            pipeline.expire(redis_key, current_app.config.get("HOUES_LIST_PAGE_REDIS_CACHE_EXPIRES"))
